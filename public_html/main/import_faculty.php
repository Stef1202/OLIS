                 ...
                 <div class="card-body">
                    ...
                    $result = $db->query($strQuery) or exit("Error code ({$db->errno}): {$db->error}");
                        ...
                        for ($i = 0; $i < count($fields); $i++){
                            if($fields[$i] != 'id' && $fields[$i] != 'role' && $fields[$i] != 'pic' && $fields[$i] != 'created_at' && $fields[$i] != 'school' && $fields[$i] != 'suffix' && $fields[$i] != 'status' && $fields[$i] != 'yr_lvl' && $fields[$i] != 'gender' && $fields[$i] != 'username' && $fields[$i] != 'password'){
                        ?>
                        <div class="form-group padd" ><label class="col-sm-2"><?= ($fields[$i]=='userNo' ? 'User Number' :  ($fields[$i]=='fname' ? 'First Name' : ($fields[$i]=='lname' ? 'Last Name' : ($fields[$i]=='mname' ? 'Middle Name' : ($fields[$i]=='yr_lvl' ? 'Year Level' : ($fields[$i]=='bdate' ? 'Birthday' : ($fields[$i]=='address' ? 'Address' : ($fields[$i]=='contact' ? 'Contact' : ($fields[$i]=='email' ? 'Email Address' : ($fields[$i]=='gender' ? 'Gender' : ($fields[$i]=='course' ? 'Department' : $fields[$i] ))))))))))); ?></label>
                                <div class="col-sm-10">
                                    <select name="<?= $fields[$i]; ?>" id="<?= $fields[$i]; ?>" class="form-control" required>
                                        <option value="NULL"></option>
                                        <?php   $str = "DESC `new_faculty_users`";
...
if (isset($_POST['import'])) {
	...
	$result->execute(); 
	 ...
	 if ($result->rowCount() > 0) {
        ...
        $r = $db->prepare($sql1);
        $r->execute();
        echo "<script>alert('$message'); window.location.href='upload_faculty.php'</script>";
        exit();
    } else {
        $sql = "INSERT INTO tbl_users(userNo, fname, lname, mname, role, address, contact, email,  course,  bdate, username, password, school, status)
                SELECT $userNo, $fname, $lname, $mname, '$role', $address, $contact, $email,  $course,  $bdate, $username, $password, '$school', '$status'
                FROM new_faculty_users";
        $q = $db->prepare($sql);
        $q->execute();
        ...
        echo '<script>alert("Imported Successfully"); window.location.href="upload_faculty.php"</script>';
        exit();
    }
}
